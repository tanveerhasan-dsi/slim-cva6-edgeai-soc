...
|---|---|
| `compliance/` | Architectural compliance configuration and results — run **twice**, with and without handlers installed (R-6.11–R-6.13). |
| `cosim/` | Co-simulation against a reference model, covering emulation handlers as well as native instructions (R-6.3, R-6.4). |
...
**Advancing on a benchmark.** A benchmark completing proves the core is not
...
Both compliance runs get published (R-5.15, R-6.13). The bare run characterises
the hardware; the handler run characterises the part. The delta between them
quantifies what was moved into software.

---

[`README`](../README.md) · [`docs/06 — Verification Requirements`](../docs/06-verification-requirements.md)
