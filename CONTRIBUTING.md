...
> [!WARNING]
> **Never edit the upstream core in place.** It is a pinned submodule. Pruning
> is configuration layered alongside it (R-3.4). Editing upstream breaks the
> pin, makes configurations non-composable, and makes equivalence against the
> baseline meaningless.
>
> **Never commit PDK-derived data.** See R-7.6–R-7.9. This one cannot be undone
> by a later commit.
...
versions (R-4.16, R-7.14). If you change the flow, re-measure everything — a
mixed results table looks like a measurement of the design when it is a
...
under the ISA heading overstates the result the project is claiming.
...
unchanged afterwards. The disagreements are the most informative part of the
work: on a project whose contribution is a method, evidence that the method
could surprise its authors is what makes it credible.

---

## Measurement and policy stay separate in `tools/isaprof`

The measurement passes measure; the policy module judges. A test enforces the
separation, and it is not a formality: the policy is expected to be replaced,
and replacing it must not require touching the measurement.

If you change the policy, say so in the methodology document too. A policy that
drifts in code while the documented method stays still is worse than either.
