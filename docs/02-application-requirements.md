...
> [!NOTE]
> **Sections 1–8 are requirements** — what must be true of the device.
> **[Section 9](#9-reference-architecture) is a reference architecture** that
> satisfies them.

**Contents:**
[1. The device](#1-the-device) ·
[2. Why the application is a requirement](#2-why-the-application-is-a-requirement-not-decoration) ·
[3. Why radar rather than a camera](#3-why-radar-rather-than-a-camera) ·
[4. Memory](#4-memory) ·
[5. Architecture](#5-architecture) ·
[6. The closed-loop requirement](#6-the-closed-loop-requirement) ·
...
| Blocker | Detail |
|---|---|
| **Weights** | An INT8 MobileNetV2-SSD or Tiny-YOLO-class detector needs roughly **3–6 MB** of weights. A realistic on-chip SRAM budget for an MPW die is a few hundred kilobytes — an order-of-magnitude shortfall, not a tuning problem. Closing it requires off-chip DRAM, hence a DRAM PHY. |
| **Ingest** | SPI cannot sustain a camera stream. MIPI CSI-2 requires a hard analogue PHY. |
...
<table>
<tr>
<th align="left" width="50%">Corpus A — portable baselines, <i>for comparability</i></th>
<th align="left" width="50%">Corpus B — target workload, <i>for the design</i></th>
</tr>
<tr valign="top">
<td>
...
> [!NOTE]
> **Reference design.** One way to satisfy §§1–8, not a specification. Where the
> team has measured data and this section has estimates, the team's judgement
> wins.
...
flowchart TD
    ...
    AO["ALWAYS-ON DOMAIN<br/>front-end + trigger<br/>uW class, never gated"]
    CORE["SLIM CVA6 (CV32A6)<br/>pruned to subset"]
    ML["Xtinyml datapath<br/>INT8 / INT4 array"]
    MEM["ON-CHIP SRAM<br/>weights, activations, code, data"]
    PWM(["PWM to motors"])
...
RV32 for tapeout; RV64 through the same harness as a scaling study
([R-1.2](01-objectives-and-scope.md#5-core-target)), so the claim is measured
...
| Retained | Detail |
|---|---|
| **FPGA target** | Kintex UltraScale+ class |
| **Staged validation matrix** | Gate by gate — [`06`](06-verification-requirements.md) |
| **Named custom instructions** | `MAT_LOAD`, `MAT_MUL`, `MAT_ACT_RELU` |
| **The physical demonstration** | Die on a daughterboard, mounted on a drone frame, benchmarked as FPS against milliwatts |

Its validation *metrics* are the part replaced. "Successfully run CoreMark" is a
smoke test, not a tapeout gate
([R-6.1](06-verification-requirements.md#2-running-coremark-is-not-a-gate)). Its
camera-based sensing and RV64 target are replaced for the feasibility reasons in
[§3](#3-why-radar-rather-than-a-camera) and [§9.3](#93-why-rv32-not-rv64).
