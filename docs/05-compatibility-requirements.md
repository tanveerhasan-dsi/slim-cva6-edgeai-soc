...
```
  illegal instruction
        |
        v
  M-mode trap  -->  decode instruction word at mepc
                       |
        +--------------+--------------+
        |                             |
   known removed encoding      genuinely illegal
        |                             |
   emulate, update arch state    escalate to the
   advance mepc by 2 or 4        application handler
        |                             |
        +-------------> mret <--------+
```
...
- `div`, `divu`, `rem`, `remu` — the most valuable, and the most likely to be
  removed for real area
...
software-visible. **Classify each removal by whether it is software-visible**
(R-3.11) — that classification determines the handler workload, and being wrong
in either direction is expensive.

## 7.3 The three failure modes

**Side effects.** Getting `div` to produce the right quotient is easy. Getting
division by zero to return the architecturally-specified value rather than
trapping is where a handler silently diverges, and where the resulting bug looks
like a compiler fault years later.

**Interrupt-context re-entrancy.** After pruning, ISRs contain emulated
instructions, so the emulator runs in interrupt context. Shared static state
corrupts under nesting, at a rate rare enough to be effectively undebuggable in
silicon. Handlers must be pure functions of trap state.

**Nested traps.** An emulated load can fault; an emulated instruction can hit a
breakpoint. Rare, therefore usually untested, therefore the one that reaches the
field.
...
**Previous:** [`04-methodology-requirements.md`](04-methodology-requirements.md) ·
**Next:** [`06-verification-requirements.md`](06-verification-requirements.md)
