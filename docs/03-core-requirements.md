...
> [!NOTE]
> **Sections 1–6 are requirements** — what must be true of the core and the
> pruning harness. **[Section 7](#7-reference-xtinyml-datapath) is a reference
> datapath** that satisfies §5.
...
[**7. Reference Xtinyml datapath**](#7-reference-xtinyml-datapath)
...
> [!NOTE]
> **Reference design.** A datapath satisfying [§5](#5-reinvestment). Dimensions
> in particular are expected to change once measured.
...
- **The memory interfaces are optional and not implemented in CVA6.** The
  coprocessor cannot issue its own loads and stores through the interface.

> [!CAUTION]
> Check the memory-interface limitation against the datapath design **early**. A
> design that assumed coprocessor-initiated memory access needs a DMA path or
> core-issued loads, and discovering that late is a redesign.
...
| Stage | Build | What it isolates |
|:--:|---|---|
| **1** | **Reference C**, no custom instructions | Correctness and the energy baseline |
| **2** | **`DOT4_I8` + `SHNR_SAT` only**, no array | The SIMD contribution |
| **3** | **Full array** | The array contribution |
...
> to hide load latency doubles the staging cost. **Size buffers and weights
> together before committing to an array dimension.**
