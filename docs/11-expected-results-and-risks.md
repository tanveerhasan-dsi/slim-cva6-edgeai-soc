...
|:--:|---|---|
| [**P-1**](#p-1--tier-1-will-generate-the-headline-and-deliver-the-least-silicon) | Tier 1 will generate the headline and deliver the least silicon | High on direction, low on magnitude |
| [**P-2**](#p-2--the-area-reduction-will-fall-short-of-the-intuitive-estimate) | The area reduction will fall short of the intuitive estimate | High |
| [**P-3**](#p-3) | A 16×16 INT8 array will not be "easily" affordable | Medium-high |
| [**P-4**](#p-4--emulation-overhead-will-be-negligible-on-average-and-unacceptable-in-the-tail) | Emulation overhead: negligible on average, unacceptable in the tail | High on the mean, medium on the tail |
| [**P-5**](#p-5--peak-activation-working-set-will-bind-before-weight-size) | Peak activation working set will bind before weight size | Medium |
| [**P-6**](#p-6--formal-equivalence-will-not-close-on-the-full-core) | Formal equivalence will not close on the full core | High |
| [**P-7**](#p-7--the-reviewer-question-will-be-why-not-ibex) | The reviewer question will be "why not Ibex?" | — |
...
| # | Risk | Mitigation |
|:--:|---|---|
| **R-1** | **The application is a moving target.** Radar front-end choice changes the network, which changes the subset, which changes the RTL. | Freeze Corpus B before D-3; treat later changes as a pipeline re-run (R-4.10), never a manual patch. |
| **R-2** | **Scope creep into mixed-signal.** Someone will propose a sensor needing a PHY. | R-2.2 is a hard boundary. This has sunk university tapeouts before. |
| **R-3** | **Toolchain drift invalidating the subset.** A compiler upgrade emits an instruction the analysis never saw. | Wire the pipeline re-run into CI ([`06`](06-verification-requirements.md) §9.4). This will otherwise be forgotten, and the failure is silent. |
| **R-4** | **Formal effort consuming the schedule.** Equivalence checking absorbs unlimited time. | Time-box; fall back to bounded checking with the bound stated. |
| **R-5** | **The demo overtaking the engineering.** A flying drone is a compelling demonstration and a large distraction — airframe, flight controller, safety, mechanical integration. | The chip result must stand on bench measurement alone. **The drone is presentation, not evidence.** |
| **R-6** | **Publishing only flattering configurations.** The strongest pressure at write-up. | R-4.18 and R-3.12 are requirements precisely so this is a compliance question rather than a judgement call under deadline. |
