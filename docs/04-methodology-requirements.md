...
> method. **[Section 9](#9-reference-method) is a reference method** that
> satisfies them.
>
...
> What is *not* negotiable is **[§2 — the deletion criterion](#2-the-deletion-criterion)**,
> because getting it wrong is not recoverable in silicon.
...
[5. Coverage of the whole workload](#5-coverage-of-the-whole-workload) ·
...
> *cannot* execute. **Coverage is a property of the stimulus, not of the
> program.** The interrupt path that only runs on a fault, the error handler that
> only runs on a sensor timeout, the libc branch that only runs on an allocation
> failure — none of these appear in a healthy run, and all of them are reachable.
> A method that treats "not observed" as "not present" will produce a chip that
> works on the bench and hangs in flight.
>
...
```mermaid
flowchart LR
    ELF["linked ELF"] --> ST["static<br/>linear sweep of every<br/>allocated executable region"]
    TRACE["simulator trace"] --> DY["dynamic<br/>execution counts"]
    ST -->|"facts"| CL["classify<br/>reference policy of §9"]
    DY -->|"facts"| CL
    CL -->|"judgement"| OUT["subset.json"]
```

> [!NOTE]
> Measurement and policy are kept in separate modules **on purpose**. `static`
> and `dynamic` are facts; `classify` is a judgement, and it is the part expected
> to be replaced. Swapping the policy must not require touching the measurement.
...
> [!NOTE]
> **Reference design.** A method satisfying §§1–8. Adopt, adapt, or replace — but
> read [§9.1](#91-the-doctrine-two-criteria-two-purposes) first, because that part
> is not a preference.
...
| Tier | What it changes | Contents | Predicted yield |
|:--:|---|---|---|
| **0** | Existing CVA6 knobs, no RTL change | FPU disabled. MMU absent in CV32A6. A-extension disabled. RVFI disabled for the silicon build. PMP entries reduced to the measured requirement. BTB/BHT/RAS and I$/D$ resized to the measured working set. | **Most of the area.** Costs almost nothing to implement — see [`11`](11-expected-results-and-risks.md). |
| **1** | New decoder-level parameters | Unused compressed subsets dropped. `div`/`rem` removed and emulated. Unread CSRs removed. Multiplier narrowed or made iterative. Unused ALU operations pruned. | **The least silicon.** The tier that generates the headline ("we removed 40% of the ISA"). Report both facts together. |
| **2** | Structural | Scoreboard entries reduced. Commit stage simplified toward single-issue. Misaligned-access support dropped. Unused functional-unit ports collapsed. | **Second-largest contributor after Tier 0.** Highest effort, highest risk to timing closure. |
...
exposing `SLIM_*` parameters.

> [!WARNING]
> **Never edit upstream RTL** — that breaks the submodule pin, destroys
> composability, and voids the equivalence proof against the baseline.
...
```mermaid
flowchart TD
    S1["1. Build Corpus B<br/>including RTOS kernel, ISRs,<br/>boot and fault paths"]
    S2["2. Static sweep every linked image<br/>union across the corpus"]
    S3["3. Dynamic trace the same corpus"]
    MK(["must-keep set"])
    FR(["frequency ranking"])
    S4["4. Complement of the must-keep set"]
    RC(["removal candidates"])
    S5["5. Group candidates by implementing structure (§9.3)"]
    S6["6. Per group: estimate saving,<br/>estimate emulation cost from step 3"]
    V{"classify"}
    S7["7. Generate configurations<br/>build, measure, plot"]
    S8["8. Record every nil result"]

    S1 --> S2 --> MK --> S4 --> RC --> S5 --> S6 --> V
    S1 --> S3 --> FR --> S6
    V -->|remove| S7
    V -->|remove-and-emulate| S7
    V -->|retain| S7
    S7 --> S8
```
...
emulation cost is incompatible with the determinism claim
([R-5.12](05-compatibility-requirements.md#4-cost)).
...
Stated because [§8](#8-deliverable) item 6 demands it, and a reference should
meet its own bar:

| Weakness | Detail |
|---|---|
| **Indirect jumps** | A linear sweep bounds what is *in* the image, not what is reachable. Computed jumps into data would defeat it — acceptable here only because the target is a static, single-application bare-metal image. |
| **Externally supplied code** | Any post-fabrication binary not analysed at design time is outside the guarantee. The compatibility contract is what makes this survivable rather than fatal. |
| **Self-modifying code** | Out of scope; state it as an assumption rather than assuming it silently. |
| **Toolchain drift** | A compiler upgrade can introduce instructions the analysis never saw. The pipeline must be re-runnable (R-4.10) and must actually be re-run on every toolchain change — a requirement that will be forgotten unless it is wired into CI. |
